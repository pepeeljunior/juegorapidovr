 ...
 *   IDLE → si jugador en rango, CHASE → si en rango melee, ATTACK
 *   HURT (knockback breve) → DEAD (despawn)
 * Construidos con geometría primitiva (no requieren assets externos).
 */
...
export class Enemies {
    ...
    _spawnAll() {
        const types = ['goblin', 'goblin', 'orc', 'wraith', 'goblin', 'orc', 'wraith'];
        for (let i = 0; i < ENEMIES.SPAWN_COUNT; i++) {
            ...
            while (!ok && tries++ < 50) {
                ...
                if (Math.sqrt(x*x + z*z) > 12) ok = true;
            }
            this._spawn(types[i % types.length], x, z);
        }
    }
    ...
    _buildGoblin(g) {
        ...
        [-1, 1].forEach(s => {
            ...
            eye.position.set(s * 0.09, 1.32, 0.21);
            g.add(eye);
        });
        ...
        [-1, 1].forEach(s => {
            ...
            leg.position.set(s * 0.12, 0.25, 0); leg.castShadow = true; g.add(leg);
        });

        // Brazos
        [-1, 1].forEach(s => {
            const arm = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.07, 0.5, 6), skin);
            arm.position.set(s * 0.32, 0.85, 0); arm.rotation.z = s * 0.3;
            g.add(arm);
        });
    ...
    _buildOrc(g) {
        ...
        [-1, 1].forEach(s => {
            ...
            tusk.position.set(s * 0.1, 1.65, 0.28);
        ...
        [-1, 1].forEach(s => {
            ...
            eye.position.set(s * 0.12, 1.78, 0.27);
            g.add(eye);
        });
        ...
        [-1, 1].forEach(s => {
            ...
            arm.position.set(s * 0.5, 1.0, 0); arm.rotation.z = s * 0.25;
            g.add(arm);
        });
        ...
        [-1, 1].forEach(s => {
            ...
            leg.position.set(s * 0.15, 0.35, 0); leg.castShadow = true; g.add(leg);
        });
    ...
    _buildWraith(g) {
        ...
        [-1, 1].forEach(s => {
            ...
            arm.position.set(s * 0.45, 1.3, 0); arm.rotation.z = s * 0.4;
            g.add(arm);
        });
    ...
    update(dt, elapsed, playerPos, cameraPos, onDamagePlayer) {
        for (const e of this.list) {
            ...
            if (e.state === STATES.ATTACK) {
                // Mantener orientación al jugador y golpear al pico del swing
    ...
    _kill(e) {
        ...
        bus.emit(EVT.ENEMY_KILLED, { type: e.type, xp: e.cfg.XP, position: e.group.position.clone() });
    }
}
